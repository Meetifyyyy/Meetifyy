...
Ordered by how easily each one slips through. The first four fail **silently** —
...
### 2. Vercel environment scopes are separate
...
### 3. `vercel.json` names the backend host literally
...
### 4. Supabase auth emails are stored in Supabase, not in this repo
...
`prisma migrate deploy` runs as the pre-deploy step (`railway.json`,
`Procfile`, and the root `npm start`). It applies committed migrations in order
...
| `admin.vercel.app` + `api.railway.app` | **no** | needs `none` + `COOKIE_SECURE=true` |
...
**2. Backend environment** (Railway/Render — runtime values, applied on restart).
Start from `backend/.env.production.example`. Confirm `APP_ENV=production`,
freshly generated secrets, and the CSP block above.

**3. Supabase dashboard** — Site URL, Redirect URLs, and re-rendered email
templates (see [#4](#4-supabase-auth-emails-are-stored-in-supabase-not-in-this-repo)).
...
| --- | --- |
| App still calls the dev API after changing `VITE_API_URL` | Not rebuilt, or set in the wrong Vercel scope — [#1](#1-frontend-variables-are-baked-in-at-build-time), [#2](#2-vercel-environment-scopes-are-separate) |
| Reset email arrives with a dev link | `FRONTEND_URL` on the backend, or the Supabase templates were never re-rendered — [#4](#4-supabase-auth-emails-are-stored-in-supabase-not-in-this-repo) |
| Reset link lands on the wrong page | Redirect URL not in Supabase's allow-list — [#4](#4-supabase-auth-emails-are-stored-in-supabase-not-in-this-repo) |
