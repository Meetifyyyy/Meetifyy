...
export default function ChatMessageList({
  ...
  const prevMessagesCountRef = useRef(loadedMessages?.length || 0);
  const prevFirstMsgIdRef = useRef(loadedMessages?.[0]?.id);
  ...
  const itemsToRender = useMemo(() => {
    ...
    const filteredMessages = (loadedMessages || []).filter((msg, idx) => {
  ...
  }, [loadedMessages, conversation?.groupUpdatesActive, conversation?.isInstantMatch, hasMore]);
  ...
  useLayoutEffect(() => {
    if (!bodyRef.current || !loadedMessages) return;
    ...
    const firstMsgId = loadedMessages[0]?.id;
    ...
    prevFirstMsgIdRef.current = firstMsgId;
    prevMessagesCountRef.current = currentCount;
  }, [loadedMessages, conversation?.id, currentUser?.id, replyingTo]);
