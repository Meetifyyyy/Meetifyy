...
describe('Content removal notifications', () => {
  ...
  const actor = { id: 'mod-1', username: 'mod', displayName: 'Mod Squad', avatar: null };
  ...
  describe('the notification itself', () => {
    ...
    it('distinguishes the owner from a moderator', () => {
      const dto = factory.createContentRemoved(actor, {
        recipientId: 'author-1', contentType: 'comment', removedBy: 'owner',
        entityId: 'k1', postId: 'p1', communityName: 'Chess Club',
      });
      expect(dto?.body).toBe('Your comment in Chess Club was removed by the community owner.');
      expect(dto?.metadata).toMatchObject({ commentId: 'k1', contentType: 'comment' });
    });

    it('reads sensibly with no community name', () => {
      const dto = factory.createContentRemoved(actor, {
        recipientId: 'author-1', contentType: 'post', removedBy: 'moderator', entityId: 'p1',
      });
    ...
    it('refuses to notify an author about their own deletion', () => {
      ...
      const dto = factory.createContentRemoved(actor, {
        recipientId: actor.id, contentType: 'post', removedBy: 'owner', entityId: 'p1',
      });
      expect(dto).toBeNull();
    });
  });
  ...
  describe('wired into the delete paths', () => {
    const build = ({ authority }: { authority: 'author' | 'owner' | 'moderator' }) => {
      const created: any[] = [];
      const prisma: any = {
        post: {
          findUnique: jest.fn(async () => ({
            id: 'p1', authorId: 'author-1', communityId: 'c1', text: 'hi',
          })),
        ...
        community: { findUnique: jest.fn(async () => ({ name: 'Chess Club' })) },
        $transaction: jest.fn(async () => []),
      };
      const notifications: any = {
        createNotification: jest.fn(async (dto: any) => { created.push(dto); }),
      };
      const authorizer: any = { assertCanDelete: jest.fn(async () => authority) };
      const service = new PostsService(
        prisma, notifications, factory, {} as any,
        { emit: jest.fn() } as any, {} as any, {} as any, {} as any, authorizer,
      );
    ...
    it('does not fail the deletion when the notification throws', async () => {
      ...
      notifications.createNotification.mockRejectedValue(new Error('queue down'));
      await expect(service.deletePost('p1', 'owner-1')).resolves.toMatchObject({ success: true });
      await settle();
    });
