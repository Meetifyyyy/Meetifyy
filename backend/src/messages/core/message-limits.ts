...
/**
 * The longest client-supplied message id the server will accept.
 *
 * `clientId` is the sender's own id for a send: the idempotency key matched
 * against the indexed `clientMessageId` column, and what lets a client merge
 * its optimistic copy with the saved message. It is generated by the client, so
 * it is bounded here for the same reason the body is — an unbounded value would
 * be stored and indexed.
 *
 * Comfortably above anything the clients generate (a timestamp plus a short
 * random suffix) and far below anything worth indexing.
 */
