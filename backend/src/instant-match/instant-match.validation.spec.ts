...
describe('Instant Match payload validation', () => {
  ...
  describe('queue:join', () => {
    ...
    it.each([null, undefined, 'nope', 42])(
      ...
      (body) => {
        expect(() => parseJoinQueuePayload('u1', body)).toThrow(
          BadRequestException,
        );
      },
    );
    ...
    it('rejects an activity outside the allow-list', () => {
      expect(() =>
        parseJoinQueuePayload('u1', { ...valid, activity: 'skydiving' }),
    ...
    it('rejects a missing activity rather than writing an empty bucket', () => {
      ...
      expect(() => parseJoinQueuePayload('u1', rest)).toThrow(
        BadRequestException,
      );
    });
    ...
    it('rejects a time preference outside the allow-list', () => {
      expect(() =>
        parseJoinQueuePayload('u1', { ...valid, timePreference: 'someday' }),
      ).toThrow('Pick when you want to meet');
    });

    it('rejects a blank campus', () => {
      expect(() =>
        parseJoinQueuePayload('u1', { ...valid, campus: '   ' }),
      ).toThrow(BadRequestException);
    });

    it('rejects an unknown campus area', () => {
      expect(() =>
        parseJoinQueuePayload('u1', {
          ...valid,
    ...
    it('truncates an over-long optional detail instead of failing the join', () => {
      const dto = parseJoinQueuePayload('u1', {
        ...valid,
        optionalDetail: 'x'.repeat(500),
      });
    ...
    it('treats a whitespace-only detail as absent', () => {
      expect(
        parseJoinQueuePayload('u1', { ...valid, optionalDetail: '   ' })
          .optionalDetail,
      ).toBeNull();
    });
    ...
    it.each([
      ...
      expect(() =>
        parseJoinQueuePayload('u1', { ...valid, location: { gps } }),
  ...
  describe('match:respond', () => {
    it('accepts both valid actions', () => {
      expect(
        parseMatchRespondPayload({ matchId: 'm1', action: 'accept' }),
      ).toEqual({ matchId: 'm1', action: 'accept' });
      expect(
        parseMatchRespondPayload({ matchId: 'm1', action: 'decline' }),
      ).toEqual({ matchId: 'm1', action: 'decline' });
    });

    it.each(['maybe', '', null, 1])('rejects action %p', (action) => {
      expect(() => parseMatchRespondPayload({ matchId: 'm1', action })).toThrow(
        BadRequestException,
      );
    });

    it('rejects a missing match id', () => {
      expect(() => parseMatchRespondPayload({ action: 'accept' })).toThrow(
        'Missing match id',
      );
    });
  });
