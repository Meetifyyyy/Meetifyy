...
describeIfEnabled(
  'Account deletion — full lifecycle against a real database',
  () => {
    ...
    afterAll(async () => {
      ...
      const steps: [string, () => Promise<unknown>][] = [
        [
          'userOtp',
          () => prisma.userOtp.deleteMany({ where: { userId: users } }),
        ],
        [
          'message',
          () =>
        ...
        [
          ...
          () =>
            prisma.comment.deleteMany({
              where: {
                OR: [{ id: { in: created.comments } }, { authorId: users }],
              },
            }),
        ],
        [
        ...
        [
          'postLike',
          () => prisma.postLike.deleteMany({ where: { userId: users } }),
        ],
        [
          'post',
          () =>
            prisma.post.deleteMany({
              where: {
                OR: [{ id: { in: created.posts } }, { authorId: users }],
              },
            }),
        ],
        [
